    ...
    public void FixTextForUI()
{
    ...
    {
        // If the text already contains connected Arabic letters, assume it's correct
        if (IsTextCorrectArabic(fixedText))
        {
        ...
        rtlText = rtlText.Replace("\r", ""); // Remove extra carriage returns

        string finalText = "";
        string[] rtlParagraphs = rtlText.Split('\n');

        tmpTextComponent.text = "";
        for (int lineIndex = 0; lineIndex < rtlParagraphs.Length; lineIndex++)
        {
            string[] words = rtlParagraphs[lineIndex].Split(' ');
